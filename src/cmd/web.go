...
func start(ctx context.Context, cmd *cli.Command) (err error) {
	...
	f.Group("", func() {
		...
		f.Post("/contact", routes.SubmitPublicContact)
		f.Post("/xc/{token}", routes.SubmitContactExchange)
	...
	f.Get("/contact", routes.PublicContactForm)
	...
	f.Group("", func() {
		...
		f.Group("", func() {
			...
			f.Post("/contact/{id}/xc", routes.CreateContactExchangeLink)
