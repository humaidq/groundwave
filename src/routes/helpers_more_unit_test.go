...
func TestNormalizeOQRSPath(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{name: "trim leading route prefix", input: "/oqrs/A%2FB-1700000000", want: "A%2FB-1700000000"},
		{name: "keep already normalized path", input: "A%2FB-1700000000", want: "A%2FB-1700000000"},
		{name: "trim surrounding whitespace", input: "  /oqrs/A%2FB-1700000000  ", want: "A%2FB-1700000000"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := normalizeOQRSPath(tt.input); got != tt.want {
				t.Fatalf("normalizeOQRSPath(%q) = %q, want %q", tt.input, got, tt.want)
			}
		})
	}
}
