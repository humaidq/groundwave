...
func parseWebDAVOrgFilePath(pathValue string, envName string, pathMissingErr error, filenameErr error, orgFileErr error) (string, string, error) {
	if pathValue == "" {
		return "", "", pathMissingErr
	}

	parsedURL, err := url.Parse(pathValue)
	if err != nil {
		return "", "", fmt.Errorf("invalid %s URL: %w", envName, err)
	}
...
func GetZKConfig() (*ZettelkastenConfig, error) {
	...
	baseURL, indexFile, err := parseWebDAVOrgFilePath(
		zkPath,
		"WEBDAV_ZK_PATH",
		ErrWebDAVZKPathNotConfigured,
		ErrWebDAVZKPathMustIncludeFilename,
		ErrWebDAVZKPathMustBeOrgFile,
	)
	if err != nil {
		return nil, err
	}
...
// GetHomeConfig loads the home wiki configuration from environment variables.
func GetHomeConfig() (*ZettelkastenConfig, error) {
	homePath := os.Getenv("WEBDAV_HOME_PATH")
	zkPath := os.Getenv("WEBDAV_ZK_PATH")
	username := os.Getenv("WEBDAV_USERNAME")
	password := os.Getenv("WEBDAV_PASSWORD")

	homeBaseURL, homeIndexFile, err := parseWebDAVOrgFilePath(
		homePath,
		"WEBDAV_HOME_PATH",
		ErrWebDAVHomePathNotConfigured,
		ErrWebDAVHomePathMustIncludeFilename,
		ErrWebDAVHomePathMustBeOrgFile,
	)
	if err != nil {
		return nil, err
	}

	zkBaseURL, _, err := parseWebDAVOrgFilePath(
		zkPath,
		"WEBDAV_ZK_PATH",
		ErrWebDAVZKPathNotConfigured,
		ErrWebDAVZKPathMustIncludeFilename,
	...
	if strings.TrimRight(homeBaseURL, "/") != strings.TrimRight(zkBaseURL, "/") {
		return nil, ErrWebDAVHomePathMustShareParentDir
	}
...
func GetNoteByIDWithBasePath(ctx context.Context, id string, basePath string) (*ZKNote, error) {
	...
	title := utils.ExtractTitle(content)
	isPublic := utils.IsPublicAccess(content)
	isHome := utils.IsHomeAccess(content)
...
// GetHomeIndexNote fetches and parses the home wiki index note.
func GetHomeIndexNote(ctx context.Context) (*ZKNote, error) {
	...
	content, err := FetchOrgFile(ctx, config.IndexFile)
	if err != nil {
		return nil, fmt.Errorf("failed to fetch home index note: %w", err)
	}

	html, err := utils.ParseOrgToHTMLWithBasePath(content, "/home")
	if err != nil {
		return nil, fmt.Errorf("failed to parse org-mode content: %w", err)
	}
