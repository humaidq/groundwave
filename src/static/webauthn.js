(function() {
    ...
    button.addEventListener("click", function() {
      ...
      withLoading(button, function() {
        ...
        return postJSON(startUrl, {}).then(function(options) {
          ...
          var destination = result.redirect || "/contacts";
          window.location.assign(destination);
        }).catch(function(err) {
          if (err && (err.name === "NotAllowedError" || err.name === "AbortError")) {
            return;
          }
          showError(button, err.message || "Verification failed");
        });
      });
    });
  }
  ...
  function init() {
    ...
    document.querySelectorAll("[data-webauthn-reauth]").forEach(handleReauth);
  }
